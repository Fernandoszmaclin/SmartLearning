...
(function () {
  ...
  (function initTasksDrawer() {
    ...
    const render = () => { syncChrome(); applyFilter(); groupAndOrder(); updateCounts(); };
  ...
  const setKind = async (blockEl, kind) => {
  ...
  const maybeSlash = (textEl, blockEl) => {
  ...
  const openSlash = (textEl, blockEl, query) => {
  ...
  const initSidebarToggle = () => { /* reservado para o botão de menu em telas pequenas */ };
  initSidebarToggle();
})();
